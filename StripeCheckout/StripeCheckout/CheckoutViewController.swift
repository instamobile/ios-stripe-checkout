...
//        paymentContext.addCardViewControllerFooterView = addCardFooter
    ...
    private func performAction(for paymentIntent: STPPaymentIntent, completion: @escaping STPPaymentStatusBlock) {
        if self.redirectContext != nil {
            completion(.error, NSError(domain: STPError.stripeDomain, code: 123, userInfo: [NSLocalizedDescriptionKey: "Should not have multiple concurrent redirects."]))
            return
        }
        ...
        if let redirectContext = STPRedirectContext(paymentIntent: paymentIntent, completion: { [weak self] (clientSecret, error) in
            ...
            if error != nil {
                ...
                STPAPIClient.shared.retrievePaymentIntent(withClientSecret: clientSecret, completion: { (retrievedIntent, retrieveError) in
                    if retrieveError != nil {
                        completion(.error, retrieveError)
                    } else {
                                ...
                                , completion: { (confirmedIntent, confirmError) in
                                    if confirmError != nil {
                                        ...
                                        if let confirmedIntent: STPPaymentIntent = confirmedIntent {
                                            ...
                                            completion(.error, NSError(domain: STPError.stripeDomain, code: 123, userInfo: [NSLocalizedDescriptionKey: "Error parsing confirmed payment intent"]))
                                        }
                                    }
                            })
                        } else {
                             completion(.error, NSError(domain: STPError.stripeDomain, code: 123, userInfo: [NSLocalizedDescriptionKey: "Error retrieving payment intent"]))
                        }
                    }
                })
            }
            ...
            completion(.error, NSError(domain: STPError.stripeDomain, code: 123, userInfo: [NSLocalizedDescriptionKey: "Unable to create redirect context for payment intent."]))
        }
    }
